    ...
    def get(self, request):
        ...
        try:
            ...
            for each_department in department:
                ...
                path = os.path.join(settings.REPORT_ROOT, filename)
                data = pandas.DataFrame(data)
                # data.to_excel(path, na_rep='', header=columnname, index=False)
                sheet_name = 'Sheet1'
                ...
                # 执行压缩文件夹操作
                absDir = settings.REPORT_ROOT
                absDir = absDir[:-1]
                zipFile = os.path.join(settings.MEDIA_ROOT, '学院中层干部民主测评结果.zip')
                compressFolder(absDir, zipFile)
        except Exception as e:
            ret.code = 0
            ret.error = '导出失败'
            return Response(ret.dict)
